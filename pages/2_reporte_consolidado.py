...
m3.metric("Lotes erróneos", total_err)
m4.metric("Fiabilidad global", f"{total_fiab:.2%}")
...
for stype in section_order:
    ...
    sc3.metric("Lotes erróneos", stats["lotes_error"])
    sc4.metric("Fiabilidad inventario", f"{stats['fiabilidad_global']:.2%}")
    ...
    disp["Fiabilidad"] = kpis["Fiabilidad"].map(lambda x: f"{x:.2%}" if x < 1 else "100%")
...
def build_excel(results, all_stats, stock_stats, all_warnings):
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        widths = [24, 18, 22, 36, 42, 50, 3, 24, 18, 22, 50]
        ...
        row = 0
        ws.merge_range(row, 0, row, 5, f"CONSOLIDADO AUDITORÍA INTERNA — {date.today().strftime('%d/%m/%Y')}", title_f)
        ws.set_row(row, 30)
        row += 2

        for c, h in enumerate(["", "Ubicaciones\nauditadas", "Lotes auditados\ntotales", "Lotes erróneos\ntotales", "Fiabilidad\ninventario", ""]):
            ws.write(row, c, h, hdr_f)
        ws.set_row(row, 35)
        row += 1
        ws.write(row, 0, "TOTAL", t_lbl); ws.write(row, 1, tu, t_num); ws.write(row, 2, tl, t_num)
        ws.write(row, 3, te, t_num); ws.write(row, 4, tf, t_pct); ws.write(row, 5, "", t_lbl)
        row += 2
        ...
        for stype in ["ALEATORIO", "CONTROL DIFERENCIADO", "MATERIAL VALIOSO"]:
            ...
            for i, (_, r) in enumerate(kpis.iterrows()):
                ...
                ws.write(row, 3, int(r["lotes_erroneos"]), d_c)
                wpct(row, 4, r["Fiabilidad"])
                # Tipo de error: obs_inventario for main table
                obs_text = r.get("obs_inventario", "")
                if isinstance(obs_text, str) and obs_text.strip().lower() == "nan":
                    obs_text = ""
                ws.write(row, 5, obs_text, obs_f)
                row += 1
            ws.write(row, 0, f"Subtotal {stype}", t_lbl)
            ws.write(row, 1, f"{stats['ubicaciones']} ubic.", t_lbl)
            ws.write(row, 2, stats["lotes_total"], t_num)
            ws.write(row, 3, stats["lotes_error"], t_num)
            ws.write(row, 4, stats["fiabilidad_global"], t_pct)
            ws.write(row, 5, "", t_lbl)
            row += 1
        ...
        if "MATERIAL VALIOSO" in all_stats:
            ...
            row += 1
            ws.merge_range(row, 0, row, 5, "RESUMEN MATERIAL VALIOSO", sub_f)
            row += 1
            ...
            ws.write(row, 2, "Fiabilidad", d_f); wpct(row, 3, vs["fiabilidad_global"])

        ws.print_area(0, 0, row + 1, 12)
        ws.set_landscape()
        ws.set_paper(9)
        ws.fit_to_pages(1, 0)
